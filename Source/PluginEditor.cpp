    ...
    thumbnailL4(512, p.getFormatManager(), thumbnailCache)
{
    ...
    // El INIT borra la selecci�n visual al hacer clic
    ...
    auto setupClearButton = [this](juce::TextButton& btn, int layerIndex) {
        ...
        // Al pasar el rat�n (hover), rojo brillante
...
void Granular_SynthAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    auto drawLayerButtons = [&](juce::Rectangle<int> area, juce::Colour color, int num, bool isSolo, bool isMute, float layerAlpha) {
        ...
        // Le restamos 25 de abajo, pero lo trasladamos (0, -3) para separarlo del borde inferior 3 p�xeles
        ...
        // MUTE/NUM BUTTON (Invertido: brilla si la capa est� ON/no muteada)
    ...
    // ==========================================================
    // --- L�GICA DE DIBUJO DE CAPAS (HYBRID RENDER + REC MODE) ---
    // ==========================================================
    auto drawLayer = [&](juce::Rectangle<int> area, juce::Colour color, float alpha, juce::AudioThumbnail& thumb, juce::String prefix, double zF, double vsR, int num, bool isSolo, bool isMute, bool isRec, juce::AudioBuffer<float>* rawBuffer, juce::Synthesiser& synth) {
        ...
        // Si est� en modo REC, atenuamos la onda al m�ximo para que quede en segundo plano (efecto fantasma)
        ...
        // ==========================================================
        // --- OVERLAY ELEGANTE DE GRABACI�N (LA RESPIRACI�N) ---
        // ==========================================================
        if (isRec) {
            ...
            // Determinar el mensaje seg�n el modo seleccionado y pedir la IP
            int recMode = (int)audioProcessor.apvts.getRawParameterValue(prefix + "R_MODE")->load();
            juce::String statusText = "";
            juce::String currentIP = audioProcessor.getLocalIPAddress(); // <--- LLAMADA M�GICA
    ...
    // Llamadas a drawLayer pas�ndole el nuevo par�metro `r1`, `r2`, etc.
    ...
    //g.setColour(juce::Colours::lightgrey.withAlpha(0.2f)); // Gris m�s claro para los presets
    ...
    // CORRECCI�N: Forzamos fuente plana
    g.setFont(juce::Font(14.0f, juce::Font::plain));
...
void Granular_SynthAudioProcessorEditor::resized()
{
    ...
    // --- LA NUEVA POSICI�N (Arriba a la izquierda de DAW) ---
    // 160 es la distancia aproximada en p�xeles de todo tu bloque derecho. 
    // Si la "X" se monta encima de "DAW", sube este n�mero a 170 o 180. Si queda muy lejos, b�jalo a 150.
    int paddingX = 170;
    int paddingY = 10;  // Separaci�n desde el "techo" de la capa
    ...
    // 2. ZONA PROTEGIDA: Aislamos la IP arriba del todo (25 p�xeles de alto)
    ipArea = topEmptyArea.removeFromTop(25);
    topEmptyArea.removeFromTop(5); // Un peque�o margen de respiraci�n
    ...
    // Ahora s�, le pasamos el rect�ngulo correcto al m�dulo
...
void Granular_SynthAudioProcessorEditor::mouseDrag(const juce::MouseEvent& event)
{
    ...
    auto handleLayerDrag = [&](juce::Rectangle<int>& area, juce::String prefix, double& vSR, double zF, std::atomic<float>& winStartRatioToStore) {
        if (isPanMode) {
            // MODO NAVEGACI�N (Arrastrar C�mara con Clic Derecho)
...
void Granular_SynthAudioProcessorEditor::changeListenerCallback(juce::ChangeBroadcaster* source)
{
    ...
    {
        // --- A: SINCRONIZACI�N DE CAPAS (El m�vil pide cambiar la vista) ---
        int requestedLayer = audioProcessor.uiLayerRequested.load();
        if (requestedLayer != 0)
        {
            activeLayer = requestedLayer; // Actualizamos la variable global

            // Actualizamos todos los m�dulos a la capa que marca el m�vil
        ...
        // --- B: ACTUALIZACI�N DE ONDAS DE AUDIO (Con tu seguro anti-bucles) ---
...
{
    // Solo repintamos la pantalla a 30 FPS para que la animaci�n REC 
    // y la aguja de reproducci�n se muevan fluidamente.
    repaint();
}
